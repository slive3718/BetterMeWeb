<body style='overflow-x:hidden;' xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html">
<style>
				...
				<div class="shadow p-3 mb-5" style='float:left;margin-left:10px; margin-bottom:30px; right:20px;background: rgba(255,255,255, 0.8)'>
					...
					<div class="row ml-2">
						...
						<div class="rating col-md-4 p-0 mt-2">
							<span >
								<a href="" class="btn-rate btn btn-warning btn-sm"  data-post_id ="<?= $post_id ?>" data-post_rate="<?=$row->getPostRating?>"><i class="fa fa-star-o"></i> <span><small style="font-weight: bold;"><?=(isset($row->getRatePercent) && !empty($row->getRatePercent))?$row->getRatePercent.' out of 5':''?></small></span></a>
					...
					<div class="card-body">
						...
						<?php if(isset($mentor_rating) && !empty($mentor_rating)){
							foreach ($mentor_rating as $rating){
								...
								?> <div class="card">
									<div class="card-body">
										<span class="float-left"><?=$rating->mentor_name;?></span> <span class="float-right"><i class="btn badge badge-primary"><b></b><i class="fa fa-star" style="color:yellow"></i><?=$rating->rating;?></></i></span>
