				...
				<div class="shadow p-3 mb-5" style='float:left;margin-left:10px; margin-bottom:30px;'>
					...
					<div class="row ml-2">
						...
						<div class="comment p-2 cursor col-md-6" id="comment_id_<?= $post_id ?>"
							...
							<span class="ml-1"><a href="<?= base_url('mentor/viewFullDiet/' . $post_id) ?>"> Comments </a> </span> <span><?=(isset($row->getCommentCount) && ($row->getCommentCount)!=0)?$row->getCommentCount:''?></span>
						</div>
					</div>
				</div>
				<?php
		...
		<div class="shadow-lg p-3 mb-5 ml-5 responsive" style="display:inline-block;right:20px;" >
						...
						<tr>
							<td>
								<span style="float:left !important; font-weight:bold;"><a
											href="<?= base_url() . 'mentor/viewFullDiet/' . $diets->post_id ?>"><?= $diets->post_title ?></a></span>
							</td>
							<td>
			...
			<div class="card-body">
				...
				<button class="btn btn-success btn-sm"
						onclick="window.location.href='<?= base_url() . 'mentor/create_thread' ?>'"
						style="float:right">
					Create a Thread
				</button>
				<br><br>
							...
							<tr>
								<td>
									<span style="float:left; font-weight:bold;"><a
												href="<?= base_url() . 'mentor/view_this_community_post/' . $community_post_id ?>">
...
<script>
	$(document).ready(function () {
		$('.like').on('click', function () {
			...
			console.log(postId);
			var base_url = "<?= base_url() . 'mentor/likeHomepagePost'?>";
